...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.generator_ready = False
        self.instructions = np.full((self.buffer_size, self.n_envs), '', dtype=object)
    ...
    def add(
        ...
        self.instructions[self.pos] = instruction
        self.pos += 1
        if self.pos == self.buffer_size:
            self.full = True
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> RolloutBufferSamples:
        ...
        return RolloutBufferSamples(*tuple(map(self.to_torch, data))), batch_inds
